...
static int _expand(mtset_t *set, int newsize)
{
	void **newr;
	void **oldr = set->items;
...
static int _canexpand(mtset_t *set)
{
...
{
	mtset_t *out = (mtset_t*)MTS_MALLOC(sizeof(mtset_t));
	if (!out)
	{
		return NULL;
	}
...
int MT_SetRemove(mtset_t *set, void *refid)
{
	int i;
